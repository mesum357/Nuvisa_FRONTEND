        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_1.6fr_1fr] gap-y-10 lg:gap-y-0 items-start">
          <div className="flex flex-col order-2 lg:order-1 lg:pr-6 lg:border-r border-white">
            <div className="flex flex-col gap-6">
              ...
              {/* Trust Badges */}
              <div className="mt-4 flex flex-wrap items-center gap-5 pt-5 border-t border-white">
          ...
          <div className="flex flex-col order-1 lg:order-2 lg:px-6 lg:border-r border-white">
            <div className="pb-4 flex items-center gap-2">
              ...
              {data.tooltip && (
                <div className="relative inline-block cursor-help group/title">
                  <Info size={14} className="text-white/30 hover:text-white transition-colors mt-[-2px]" />
                  <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-56 p-2.5 bg-[#6F48FF] text-white text-[10px] font-gilroy-medium rounded-lg shadow-2xl opacity-0 invisible group-hover/title:opacity-100 group-hover/title:visible transition-all duration-300 z-50">
                    {data.tooltip}
                    <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-1 border-8 border-transparent border-t-[#6F48FF]"></div>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col mt-4">
              ...
              >
                ...
                {comparisonColumns.map((col, idx) => (
                  <div key={idx} className={`flex justify-center px-4 py-4 relative ${idx === 0 ? 'bg-white/5 rounded-t-2xl border-x border-t border-white/10' : ''}`}>
                    <p className={`text-[10px] md:text-[11px] font-gilroy-bold uppercase tracking-wider text-center ${idx === 0 ? 'text-[#6F48FF]' : 'text-white/60'}`}>
                      {col}
                    </p>
                  </div>
                ))}
              ...
              <div className="flex flex-col">
                {comparisonRows.map((row, rIdx) => (
                  <div
                    key={rIdx}
                    className="grid border-b border-white/10 items-stretch group"
                    style={{ gridTemplateColumns: `1.5fr repeat(${comparisonColumns.length}, 1fr)` }}
                  >
                    <div className="flex items-center gap-2 pr-2 py-5">
                      ...
                      {row.tooltip && (
                        <div className="relative inline-block cursor-help group/tip">
                          <Info size={11} className="text-white/30 hover:text-white transition-colors" />
                          <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-48 p-2.5 bg-[#6F48FF] text-white text-[10px] font-gilroy-medium rounded-lg shadow-2xl opacity-0 invisible group-hover/tip:opacity-100 group-hover/tip:visible transition-all duration-300 z-50">
                            {row.tooltip}
                            <div className="absolute top-full left-1/2 -translate-x-1/2 -mt-1 border-8 border-transparent border-t-[#6F48FF]"></div>
                          </div>
                        </div>
                      ...
                      return (
                        <div
                          ...
                          className={`flex justify-center px-4 py-5 relative h-full items-center ${cIdx === 0 ? 'bg-white/5 border-x border-white/10' : ''
                            } ${cIdx === 0 && rIdx === comparisonRows.length - 1 ? 'rounded-b-2xl border-b' : ''
                            }`}
                        >
                          {isValueBullet ? (
                            ...
                            <p className={`text-[10px] md:text-[11px] font-gilroy-medium text-center ${cIdx === 0 ? 'text-white font-gilroy-bold' : 'text-white/70'}`}>
                              {val}
                            </p>
                          )}
                        </div>
              ...
              <div className="flex flex-col gap-4">
                <ul className="space-y-4">
                  {data.experienceItems.map((task, idx) => (
                    <li key={idx} className="flex items-start gap-3 p-3 rounded-xl bg-white/5 border border-white/10 hover:border-[#6F48FF]/50 transition-colors">
