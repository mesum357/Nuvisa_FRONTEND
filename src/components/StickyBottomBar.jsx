...
const StickyBottomBar = ({ triggerElementId }) => {
  ...
  const visaFeePerTraveler = useMemo(() => {
    ...
    visaState.selectedVisaType?.priceGBP,
    visaState.selectedVisaType?.price,
    countryPricingList,
    items[0].currentPrice,
  ]);
  ...
  const discountedPrices = useMemo(() => {
    ...
    // Apply GROUP20 coupon independently
    if (hasGroupDiscount) {
      if (
        canUseReduxVisaFees &&
        travelersQualify &&
        ...
        finalVisaFees = finalVisaFees - (finalVisaFees * 20) / 100;
      }
      if (
        insuranceQualify &&
        recommendedItems?.insuranceCertificate &&
      ...
      if (
        ...
        finalGiftCardFees = finalGiftCardFees - (finalGiftCardFees * 20) / 100;
      }
    }
  ...
  return (
    <>
      <div
        ref={barRef}
        className={`fixed bottom-0 left-0 right-0 z-[70] bg-[#1e1e27] ${
          isDrawerOpen ? "shadow-none" : "shadow-2xl"
        } animate-slide-up`}
      >
        <div className="mx-auto px-4 sm:px-10 py-4">
          ...
          <div className="hidden lg:flex sm:flex-wrap md:items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3 text-white">
              {items
                .filter((item) => item.id === "schengen")
                .map((item) => (
                  <div
                    key={item.id}
                    className="flex flex-col xl:flex-row items-center gap-3 text-white"
                  >
                    {/* Item Info */}
                      ...
                      <div className="flex items-end gap-2 ">
                        ...
                        <div className="flex flex-col">
                          ...
                          <div className="flex items-center gap-2">
                            <span className="text-gray-400 line-through text-sm">
                              £
                              {quantities[item.id] > 0
                                ? item.originalPrice * quantities[item.id]
                                : item.originalPrice}
                            </span>
                            <span className="text-white font-bold">
                              £
                        ...
                        <div className="xl:ml-4">
                          <label className="relative inline-flex items-center cursor-pointer">
                            ...
                            />
                            <div
                              className="w-11 h-6 bg-gray-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"
                              style={{
                                "--tw-bg-opacity":
                                  quantities[item.id] > 0 ? "1" : "0",
                                backgroundColor:
                                  quantities[item.id] > 0
                                    ? "#6B4EFF"
                                    : "#6b7280",
            ...
            <div className="mb-3 relative z-[80]">
              {items
                ...
                .map((item) => (
                  <div
                    key={item.id}
                    className="bg-[#24242D] rounded-2xl px-4 py-3"
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2 relative z-[90]">
                        <button
                          ...
                          style={{
                            backgroundColor:
                              quantities[item.id] === 0 ? "#6b7280" : "#6B4EFF",
                          }}
                          onMouseEnter={(e) => {
                            if (quantities[item.id] > 0) {
                              e.target.style.backgroundColor = "#5A3FE6";
      ...
      >
        <div className="space-y-4">
          {items
            .filter((item) => item.id === "insurance" || item.id === "giftCard")
            .map((item) => (
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-white font-bold">
                        £
                        {quantities[item.id] > 0
                          ? Math.round(getItemDiscountedPrice(item.id))
                          : item.currentPrice}
                      </span>
                    </div>
                  </div>
                  ...
                  <label className="relative inline-flex items-center cursor-pointer">
                    ...
                    <div
                      className="w-11 h-6 bg-gray-600 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all"
                      style={{
                        "--tw-bg-opacity": quantities[item.id] > 0 ? "1" : "0",
                        backgroundColor:
                          quantities[item.id] > 0 ? "#6B4EFF" : "#6b7280",
                      }}
                    ></div>
                  </label>
                </div>
